...
import 'city_tour_type.dart';

class UseTypeFour extends StatefulWidget {
  const UseTypeFour(
      {super.key,
  ...
  @override
  State<UseTypeFour> createState() => _UseTypeFourState();
}

class _UseTypeFourState extends State<UseTypeFour>
  ...
  // dynamic addTourAmount = 0;
  dynamic totalListPrice = 0;
  dynamic PackagePrice = 0;
  dynamic totalFinalTax = 0;
  ...
  void initState() {
    ...
    print("${widget.extraTransportPrice}");
    ...
    // Handle person/cab initialization
  ...
  void addCabQuantity(int index) {
    setState(() {
      ...
      // ✅ Step 2: Rebuild cabFinalAmount safely
      ...
      // ✅ Step 3: Now it's safe to access index
      calculateGst(cabFinalAmount[index], widget.tourGst);

      // ✅ Step 4: Final state updates
  ...
  void increaseQuantity(int index, dynamic currentPrice, dynamic cabSeats) {
    if (cabsQuantity[index] < cabSeats) {
      ...
      setState(() {
        ...
        int totalPrice = (currentPrice * cabsQuantity[index]);
        cabFinalAmount[index] = totalPrice; // Update the final amount list
        calculateGst(cabFinalAmount[index], widget.tourGst);
        addFoodItem(
            widget.cabsquantity![index].enCabName ?? '',
            cabFinalAmount[index],
  ...
  void decreaseQuantity(int index, dynamic currentPrice) {
    ...
    setState(() {
      ...
      int totalPrice = (currentPrice * cabsQuantity[index]);
      cabFinalAmount[index] = totalPrice;
      calculateGst(cabFinalAmount[index], widget.tourGst);
      addFoodItem(
          widget.cabsquantity![index].enCabName ?? '',
          cabFinalAmount[index],
  ...
  void addPersonQuantity(int index) {
    setState(() {
      ...
      // ✅ Always rebuild cabFinalAmount (outside the loop)
      cabFinalAmount.clear();
      for (int i = 0; i < widget.cabsquantity!.length; i++) {
        int currentPrice =
            int.tryParse(widget.cabsquantity![i].price ?? '0') ?? 0;
      ...
      // ✅ Safe assignment after list is built
  ...
  void sumTotal() {
    ...
    for (var item in packageItems) {
      ...
      final taxPrice =
          double.tryParse(item["tax_price"].toString().trim()) ?? 0.0;

      totalListPrice += price;
      //addTourTaxAmount += taxPrice;
    }
                ...
                children: [
                  ...widget.services!.map((service) {
                    if (service == "transport") {
                                    ...
                                    ListView.builder(
                                      ...
                                      itemBuilder: (context, index) {
                                        ...
                                        return CabInfoCard(
                                          ...
                                          personInCab: true,
                                          includedStatus: widget.foodList[index].includedStatus,
                                          //hotelTypeList: [],
                                        );
                                      },
                                    ),
                      ...
                      return const Text("empty"); //Fallback for unknown services
                    }
                  }).toList(),
                        ...
                        children: [
                                        ...
                                        children: [
                                          ...
                                          Text(
                                              selectedCabTotalAmount == 0
                                                  ? "INCLUDED"
                                                  //: "Total: ₹${selectedCabTotalAmount!.toStringAsFixed(2)}",
                          ...
                          if (onlyPickUp || onlyDrop || both)
                            ListView.builder(
                              ...
                              physics: const NeverScrollableScrollPhysics(),
                              itemBuilder: (context, index) {
                                //addTransportTaxAmount = distanceArray[index]["tax_price"];
                                return Container(
                                  margin: const EdgeInsets.all(5),
                                  padding: const EdgeInsets.all(10),
                                  decoration: BoxDecoration(
                                    ...
                                    children: [
                                      Column(
                                        ...
                                        children: [
                                          ...
                                          // distanceArray[index]["tax_price"] == "" ? SizedBox() :
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          selectOrder == widget.services!.length + 1
                              ...
                              : totalAmountDisplay(
                                  amount:
                                      "${specificCabTotal + specificFoodTotal + specificHotelTotal + (int.tryParse("$selectedExPrice"))}",
                                ),
                          const SizedBox(height: 10),
