...
class ProductDetailsController extends ChangeNotifier {
      ...
      if (_productDetailsModel != null) {
        ...
        Provider.of<SellerProductController>(Get.context!, listen: false)
            .getSellerProductList(
                productDetailsModel!.userId.toString(), 1, productId);
      }
    } else {
      _isDetails = false;
  ...
  getSharableLink(String productSlug, BuildContext context) async {
    ApiResponse apiResponse =
        await productDetailsServiceInterface.getSharableLink(productSlug);
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
      _sharableLink = apiResponse.response!.data;
      print('Sharable Link Response: ${apiResponse.response!.data}');
    } else {
      ApiChecker.checkApi(apiResponse);
    }
  }
