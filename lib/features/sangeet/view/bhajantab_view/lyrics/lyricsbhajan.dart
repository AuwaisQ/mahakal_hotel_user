...
class _LyricsbhajanState extends State<Lyricsbhajan> {
  ...
  void _shareContent() {
    ...
    // Agar SplashController use kar rahe ho to dynamically URLs fetch kar sakte ho
    var splashController =
        Provider.of<SplashController>(context, listen: false);
    playStoreUrl =
        splashController.configModel?.userAppVersionControl?.forAndroid?.link ??
            playStoreUrl;
    appStoreUrl =
        splashController.configModel?.userAppVersionControl?.forIos?.link ??
            appStoreUrl;

    String musicName = widget.musicName ?? 'संगीत';
    String cleanedLyrics = HtmlParser.parseHTML(widget.musicLyrics ?? '').text;

    // Construct the share message with both URLs
    String contentToShare = "🎶 **$musicName** 🎵\n\n"
        "\n\"$cleanedLyrics\"\n\n"
        "📜 **शब्दों की मिठास:**\n"
        "अब सुनें **महाकाल ऐप** पर! 🔱✨\n"
        "अभी डाउनलोड करें और भक्ति संगीत का आनंद लें! 🎧💖\n\n"
