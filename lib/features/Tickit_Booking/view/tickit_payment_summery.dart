...
import '../controller/tickit_booking_controller.dart';
import '../controller/tickit_leadupdate_controller.dart';

class TicketPaymentSummary extends StatefulWidget {
  ...
  final List<String> seatNumbers; // Format: ["A-2", "D-6", "F-3"]
  ...
  @override
  State<TicketPaymentSummary> createState() => _TicketPaymentSummaryState();
}

class _TicketPaymentSummaryState extends State<TicketPaymentSummary> {
  final razorpayService = RazorpayPaymentService();

  // Wallet and Coupon variables
  // double walletPay = 0;
  ...
  final fetchCouponController = Provider.of<FetchCouponController>(Get.context!, listen: false);
  ...
  void initState() {
    super.initState();
    fetchCouponController.fetchCoupon(type: "event", couponUrl: "${AppConstants.tourCouponUrl}");
  ...
  void prepareFinalDevotees() {
    ...
    int loopLength = seatLength > 0
        ? seatLength
        : devoteeLength; // priority seats ko

    for (int i = 0; i < loopLength; i++) {

      Map<String, String>? devotee = i < devoteeLength ? widget.devotees[i] : null;
  ...
  Future<void> processPayment() async {
    final bookingController = Provider.of<TickitBookingController>(context, listen: false);
    ...
    if (remainingAmount == 0 && widget.walletPay >= widget.totalAmount) {
      ...
      bool success = await bookingController.bookingSuccess(
        leadId: widget.leadId.toString(),
        walletType: "$successAmountStatus", // 1 = Wallet
      ...
      if (success) {
        ...
        showDialog(
          ...
          builder: (context) => bookingSuccessDialog(
            ...
            message: 'Your activity tickets have been successfully booked.',
          ),
    ...
    //  Wallet + Online Payment
      ...
      razorpayService.openCheckout(
        ...
        onSuccess: (response) async {
          ...
          bool success = await bookingController.bookingSuccess(
            leadId: widget.leadId.toString(),
            walletType: "$successAmountStatus", // 2 = Wallet + Online
            transactionId: response.paymentId ?? "",
            onlineAmount: remainingAmount.toString(),
          );
    ...
    //  Online Only Payment
      ...
      razorpayService.openCheckout(
        ...
        onSuccess: (response) async {
          ...
          bool success = await bookingController.bookingSuccess(
            leadId: widget.leadId.toString(),
            walletType: "$successAmountStatus", // 0 = Online
            transactionId: response.paymentId ?? "",
            onlineAmount: remainingAmount.toString(),
          );
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Text(
              remainingAmount == 0 ? "Total Amount: ${NumberFormat.currency(locale: 'en_IN', symbol: '₹', decimalDigits: 0).format(deducatedFromWallet)}" :
