...
import 'package:mahakal/features/Tickit_Booking/model/tickit_availiblity_model.dart' hide Row, LayoutRow, BlockedSeat;
...
class EventLayoutScreen extends StatefulWidget {
  final EventAuditoriumModel ticketData;
  ...
  @override
  State<EventLayoutScreen> createState() => _EventLayoutScreenState();
}

class _EventLayoutScreenState extends State<EventLayoutScreen> {
  EventAuditorium? get venue => widget.ticketData.data?.auditorium;
    ...
    if (venue?.rows != null) {
      for (var row in venue!.rows) {
        ...
        if (!seatTypes.containsKey(type)) {
          seatTypes[type] = {
            'color': hexToColor(row.color?.toString() ?? "#FFFFFF"),
                ...
                children: [
                  ...
                  ...seatTypes.entries.map((entry) {
                    ...
                    return Padding(
                      ...
                      child: _legendItem(entry.value['name'], color, false),
                    );
                  }).toList(),
                ],
              ),
  ...
  List<Widget> _buildSeatRow(
    ...
    Color seatColor = hexToColor(row.color?.toString() ?? "#FFFFFF");
    double seatPrice = double.tryParse(row.price?.toString() ?? "0") ?? 0.0;
    ...
    for (int seatIndex = 0; seatIndex < seatsPerRow; seatIndex++) {
      ...
      bool isBlocked = blockedSeats.any(
            (b) => b.row.toString() == rowIndex.toString() &&
            b.seat.toString() == seatNo.toString(),
      );

      String seatId = "$rowIndex-$seatNo";
      ...
      Widget seat = _buildSeat(
        ...
        seatNo: seatNo,
        isBlocked: isBlocked,
        isSelected: selectedSeats.any((s) => s['id'].toString() == seatId),
        color: seatColor,
        rowData: row,
        isWheelchair: false,
        seatPrice: seatPrice,
                    ...
                    Navigator.push(
                      ...
                      MaterialPageRoute(
                        builder: (context) => EventPaymentScreen(
                          ...
                          packageName: "${selectedSeats.first['type']}",
                          personCount: selectedSeats.length,
                          seatNumbers: [],
                          totalAmount: total,
                          amount: selectedSeats.first['price'],
                          devotees: widget.devotees,
                          walletPay: widget.walletPay,
                        ),
                      ),
                    );
                    },
