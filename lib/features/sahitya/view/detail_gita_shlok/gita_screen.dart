...
import 'package:mahakal/features/sahitya/controller/audio_controller.dart';
...
class _GitaScreenState extends State<GitaScreen> {
  ...
  Future<void> getChapters() async {
    print("Sahitya Verse ${widget.myId} $_verse");
    ...
    try {
      ...
      for (int i = 1; i < (_verse > 4 ? 5 : 10); i++) {
        final url = "${AppConstants.sahityaVerseUrl}${widget.myId}&verse=$_verse";
        ...
        if (res != null) {
          ...
          if (chaptersModel.data.isNotEmpty) {
            ...
            hiChapName = chaptersModel.data[0].hiChapterName;
            enChapName = chaptersModel.data[0].chapterName;
            totalVerse = chaptersModel.data[0].verseCount;

            // Avoid duplicating verses in chapterData
            if (!chapterData.contains(newVerse)) {
              tempData.add(newVerse);
      ...
      // Check if the fetched data is incomplete
  ...
  Future<void> saveDataLocally(List<Verse> data, int? chapterId) async {
    try {
      ...
      final chapterFile = File(
          '${directory.path}/chapter_$chapterId.json'); // Save each chapter in a separate file
      ...
      for (var verse in data) {
        if (verse.verseData?.audioUrl != null) {
          final audioFile =
              File('${directory.path}/chapter_$chapterId${verse.verse}.mp3');
          if (!await audioFile.exists()) {
            final response =
                await http.get(Uri.parse(verse.verseData!.audioUrl!));
            await audioFile.writeAsBytes(response.bodyBytes);
            print(
                "Audio for verse ${verse.verse} in chapter $chapterId saved locally");
          }
          verse.verseData?.audioUrl =
              '${directory.path}/chapter_$chapterId${verse.verse}.mp3';
        }
      }
    } catch (e) {
...
// Check for internet connectivity and handle accordingly
  ...
  Future<void> loadChaptersFromLocal(int? chapterId) async {
    ...
    try {
      final directory = await getApplicationDocumentsDirectory();
      final chapterFile = File(
          '${directory.path}/chapter_$chapterId.json'); // Load the specific chapter file
  ...
  Future<void> checkLocalData(int chapterId) async {
    ...
    if (await file.exists()) {
      ...
      if (jsonData.length < widget.verseCount!) {
        print(
            "Incomplete data found, deleting and refetching from first verse.");
        ...
        if (await isConnected()) {
          await loadChaptersFromAPI(
              chapterId); // Fetch new data from the first verse
        } else {
  ...
  void handleNoInternet() {
    ...
    Future.delayed(const Duration(seconds: 2), () {
      Navigator.push(
        context,
        CupertinoPageRoute(
            builder: (context) => SahityaChapters(isToast: _hasInternet)),
      );
    });
  }
  ...
  Future<void> fetchChapters() async {
    int chapterId =
        widget.myId ?? 1; // Default to chapter 1 if no chapterId is provided
  ...
  void didChangeDependencies() {
    ...
    audioManager = Provider.of<AudioPlayerManagerSahitya>(context);
  }
  ...
  void initState() {
    ...
    audioManager = Provider.of<AudioPlayerManagerSahitya>(context, listen: false);
                    ...
                    children: [
                      ...
                      Consumer<SettingsProvider>(
                        builder: (BuildContext context, settingsProvider,
                          ...
                          return GestureDetector(
                            onTap: () {
                              settingsProvider.onBookFormate();
                            },
                            child: Container(
                              width: screenWidth * 0.12,
                              height: screenWidth * 0.06,
                              decoration: BoxDecoration(
                                ...
                                color: settingsProvider.showBookFormate
                                    ? Colors.orange
                                    : CustomColors.clrblack,
                              ),
                              child: Stack(
                                children: [
                                  AnimatedPositioned(
                                    duration: const Duration(milliseconds: 300),
                                    curve: Curves.easeInOut,
                                    left: settingsProvider.showBookFormate
                                        ? screenWidth * 0.06
                                        : 0,
                                    child: Padding(
                                      padding: EdgeInsets.symmetric(
            ...
            home: SafeArea(
              child: WillPopScope(
                onWillPop: () async {
                  ...
                  Provider.of<AudioPlayerManagerSahitya>(context, listen: false)
                      .stopMusic();
                  return true;
                },
                child: Scaffold(
                                                                    ...
                                                                    Container(
                                                                      decoration: BoxDecoration(
                                                                          shape: BoxShape
                                                                              .circle,
                                                                          border: Border.all(
                                                                              color: SettingProvider.isOn ? CustomColors.clrwhite : CustomColors.clrblack,
                                                                              width: 0.9)),
                                                                      child:
                                                                          Padding(
                                                                      ...
                                                                      builder: (BuildContext
                                                                        ...
                                                                        return Text(
                                                                          ...
                                                                          textAlign:
                                                                              TextAlign.center,
                                                                          style: TextStyle(
                                                                              fontSize: settingProvider.fontSize,
                                                                              color: settingProvider.isOn ? settingProvider.textColor : CustomColors.clrblack,
                                                                              fontFamily: settingProvider.selectedFont),
                                                                        );
                                                                      },
                                                                    ),
                                                                    Padding(
                                                                          ...
                                                                          children: [
                                                                                  ...
                                                                                  Navigator.push(
                                                                                      ...
                                                                                      DialogRoute(
                                                                                        ...
                                                                                        builder: (context) => ShareScreen(
                                                                                          gitaShlok: chapterData[index].verseData?.verseData!.sanskrit ?? '',
                                                                                          shlokMeaning: chapterData[index].verseData?.verseData!.hindi ?? '',
                                                                                          detailsModel: chapterData[index],
                                                                                          verseSerial: chapterData[index].verse,
                                                                                        ),
                                                                                      ));
                                                                                },
                                                                                child: Icon(
                                                                                  Icons.share_outlined,
                                                                                  color: SettingProvider.isOn ? CustomColors.clrwhite : CustomColors.clrbrown,
                                                                            ...
                                                                            Consumer<BookmarkProvider>(
                                                                              builder: (BuildContext context, bookmarkProvider, Widget? child) {
                                                                                final isBookmarked = bookmarkProvider.bookMarkedShlokes.any((bookmarked) => bookmarked.verseData?.audioUrl == chapterData[index].verseData?.audioUrl);

                                                                                return GestureDetector(
                                                                                  onTap: () {
                                                                                    bookmarkProvider.toggleBookmark(chapterData[index]);
                                                                                  },
                                                                                  child: Icon(
                                                                                  ...
                                                                                  return GestureDetector(
                                                                                      onTap: () {
                                                                                        Navigator.push(
                                                                                            context,
                                                                                            CupertinoPageRoute(
                                                                                              builder: (context) => BhawarthScreen(
                                                                                                musicLyrics: chapterData[index].hiDescription,
                                                                                                musicName: enChapName,
                                                                                              ),
                                                                                            ));
                                                                                      },
                                                                                      child: Text(
                                                                            ...
                                                                            const Spacer(),
                                                                            Consumer<AudioPlayerManagerSahitya>(
                                                                              builder: (BuildContext context, audioController, Widget? child) {
                                                                                bool isCurrentSongPlaying = audioController.isPlaying && audioController.currentMusic == chapterData[index];
                                                                                ...
                                                                                return IconButton(
                                                                                  onPressed: () {
                                                                                    if (isCurrentSongPlaying) {
                                                                                      ...
                                                                                      Provider.of<AudioPlayerManager>(context, listen: false).stopMusic();
                                                                                    } else {
                                                                                      if (audioController.currentMusic == chapterData[index]) {
                                                                                        // Resume the current song
                                                                                        audioController.togglePlayPause();
                                                                                        Provider.of<AudioPlayerManager>(context, listen: false).stopMusic();
                                                                                      } else {
                                                                                        // Play the selected song
                                                                                        audioController.playMusic(chapterData[index], widget.myId);
                                                                                        Provider.of<AudioPlayerManager>(context, listen: false).stopMusic();
                                                                                    ...
                                                                                    print('isPlaying: ${audioController.isPlaying}');
                                                                                    print('currentMusic: ${audioController.currentMusic}');
                                                                                  },
                                                                                  icon: Icon(
                                                                                    // Determine icon based on the state of the current song
                                                                                    isCurrentSongPlaying ? Icons.pause_circle : Icons.play_circle,
                                                                ...
                                                                children: [
                                                                  ...
                                                                  Visibility(
                                                                    ...
                                                                    child:
                                                                      ...
                                                                      children: [
                                                                        Text(
                                                                          chapterData[index].verseData!.verseData?.english ??
                                                                              '',
                                                                          textAlign:
                                                                              TextAlign.center,
                                                                          style: TextStyle(
                  ...
                  bottomNavigationBar: Consumer<AudioPlayerManagerSahitya>(
                    builder: (context, musicManager, child) {
                      if (audioManager.isMusicBarVisible) {
                        return MusicBar(
                          chapterHindiName: hiChapName,
