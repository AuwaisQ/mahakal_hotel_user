...
class SliversExample extends StatefulWidget {
  ...
  SliversExample(
      {super.key, required this.slugName});
...
class _SliversExampleState extends State<SliversExample>
  ...
  void initState() {
    super.initState();
    print('date checker ${nextDatePooja}');
    getDetailsData();
  ...
  String productType = '';
  String thumbnail = '';
  String nextDatePooja = '';
  String targetDays = '';
  Duration countdownDuration = Duration.zero;
  Timer? countdownTimer;
  DateTime? parsePoojaDate() {
    if (nextDatePooja.isEmpty) return null;

    return DateFormat('yyyy-MM-dd HH:mm:ss').parse(nextDatePooja);
  }
  ...
  Future<void> getDetailsData() async {
    try {
      final url = Uri.parse(AppConstants.baseUrl +
          AppConstants.poojaSingleDetailUrl +
          widget.slugName);
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          // ------------------ IMAGE EXTRACT ------------------
          String rawImageString = res['data']['image'];
          imageList.clear(); // clear previous images if any

          // Extract JSON list from inside the string
          final regex = RegExp(r'\[(.*?)\]');
          final match = regex.firstMatch(rawImageString);

          if (match != null) {
            String jsonArrayString = '[${match.group(1)}]';
            jsonArrayString =
                jsonArrayString.replaceAll('\\', ''); // remove backslashes
            List<dynamic> decodedImages = json.decode(jsonArrayString);
            // String baseUrl = "https://mahakal.rizrv.in/storage/app/public/pooja/";
            String baseUrl =
                '${AppConstants.baseUrl}/storage/app/public/pooja/';

            imageList = decodedImages.map<String>((e) => '$baseUrl$e').toList();
          }
          ...
          enVenue = res['data']['en_pooja_venue'] ?? '';
          ...
          hiVenue = res['data']['hi_pooja_venue'] ?? '';
          ...
          nextDatePooja = res['data']['next_pooja_date'] ?? '';
  ...
  void startCountdown() {
    final targetDate = parsePoojaDate();
    if (targetDate == null) return;

    countdownTimer?.cancel(); // safety

    countdownTimer = Timer.periodic(const Duration(seconds: 1), (timer) {
      final now = DateTime.now();
      countdownDuration = targetDate.difference(now);
  ...
  void updateNextPoojaDate() {
    final currentDate = parsePoojaDate();
    ...
    setState(() {
      nextDatePooja =
          DateFormat('yyyy-MM-dd HH:mm:ss').format(nextDate);
    });
  }
  ...
  Future<void> getLeadSave(int index) async {
    ...
    const String apiUrl = '${AppConstants.baseUrl}${AppConstants.poojaLeadUrl}';
    ...
    Map<String, dynamic> data = {
      ...
      'noperson': package[index].person.toString(),
      'person_phone': userPhone,
      'person_name': userName,
      'booking_date': formatDateString(nextDatePooja)
    };
    ...
    try {
      ...
      if ([200, 201].contains(response.statusCode)) {
        ...
        // data: package, product: product, serviceId: serviceId, productType: productType, typePooja: 'pooja', poojaName: enName, poojaVenue: enVenue, date: , translateEn: translateEn,
        Navigator.push(
            context,
            CupertinoPageRoute(
              builder: (context) => Newbilldetails(
                ...
                product: product,
                personCount: package[index].person,
                typePooja: 'pooja',
                orderId: serviceId,
  ...
  Widget build(BuildContext context) {
    DateTime? date = parsePoojaDate();

    if (date == null) {
      return MahakalLoadingData(onReload: () {}); // or loading widget
    }
    String dayOfWeek = date != null ? DateFormat('EEEE').format(date) : '';
                                    ...
                                    children: [
                                      ...
                                      Row(
                                        children: [
                                          ...
                                          Text(
                                            "${DateFormat('dd-MMMM-yyyy').format(DateFormat('yyyy-MM-dd').parse(nextDatePooja))} $dayOfWeek",
                                            style: const TextStyle(
                                                fontSize: 18,
                                          ...
                                          Container(
                                              ...
                                              decoration: BoxDecoration(
                                                  ...
                                                  color: const Color.fromRGBO(
                                                      255, 238, 211, 1)),
                                              child: Text(
                                                '${countdownDuration.inHours % 24} Hour',
                                                style: const TextStyle(
                                                    fontSize: 20,
                                                    fontWeight: FontWeight.bold,
                                                    fontFamily: 'Roboto',
                                          ...
                                          Container(
                                              ...
                                              decoration: BoxDecoration(
                                                  ...
                                                  color: const Color.fromRGBO(
                                                      255, 238, 211, 1)),
                                              child: Text(
                                                '${countdownDuration.inMinutes % 60} Mint',
                                                style: const TextStyle(
                                                    fontSize: 20,
                                                    fontWeight: FontWeight.bold,
                                                    fontFamily: 'Roboto',
                                          ...
                                          Container(
                                              ...
                                              decoration: BoxDecoration(
                                                  ...
                                                  color: const Color.fromRGBO(
                                                      255, 238, 211, 1)),
                                              child: Text(
                                                '${countdownDuration.inSeconds % 60} Sec',
                                                style: const TextStyle(
                                                    fontSize: 20,
                                                    fontWeight: FontWeight.bold,
                                                    fontFamily: 'Roboto',
...
DevoteesCountWidget(),
                                ...
                                //   key:fiveKey,
                                //   child: Newpackage(
                                //     data: package, product: product, serviceId: serviceId, productType: productType, typePooja: 'pooja', poojaName: enName, poojaVenue: enVenue, date: "${widget.nextDatePooja}", translateEn: translateEn,
                                //   ),
                                // ),
