...
class VipDetails extends StatefulWidget {
  ...
  @override
  _VipDetailsState createState() => _VipDetailsState();
}

class _VipDetailsState extends State<VipDetails> with TickerProviderStateMixin {
  ...
  void initState() {
    super.initState();
    print("My VIP ID:${widget.idNumber}");
    userName =
  ...
  Future<void> getDetailsData() async {
    try {
      final url = Uri.parse(
          AppConstants.baseUrl + AppConstants.vipDetailsUrl + widget.idNumber);
      final response = await http.get(url);
      print("My VIP Url: ${url}");
  ...
  Future<void> getVipLeadSave(int index, String date) async {
    // Define the API endpoint
    const String apiUrl = '${AppConstants.baseUrl}${AppConstants.vipLeadUrl}';
    ...
    try {
      ...
      if ([200, 201].contains(response.statusCode)) {
        ...
        // Handle error
        print('Failed to book vip: ${response.statusCode}');
        print(response.body);
      }
    } catch (e) {
      // Handle exception
...
DevoteesCountWidget(),
                            ...
                            //     dataUrl: 'review-vip-pooja/$poojaSlug',
                            ...
                            Container(
                              key: sevenKey,
                              child: Askquestions(
                                  type: 'VIP Puja', translateEn: translateEn),
                            ),
                          ],
                        ),
                      ),
